...
using eProjectSemester3_1.Controllers;
...
namespace eProjectSemester3_1.Controllers
{
    ...
    {
        public FileManagerController( MembershipService membershipSevice, UnitOfWorkManager unitOfWorkManager, LoggingService loggingService)
        ...
        public ActionResult DeleteDir(string d)
        {
            ...
            if (!Directory.Exists(d))
                ...
                return Json(GetErrorRes(LangRes("E_CannotDeleteRoot")));
            else if (d == FixPath("/Content/UserFile/" + LoginUser.Id + "/"))
                return Json(GetErrorRes(LangRes("E_CannotDeleteRoot")));
        ...
        protected void CheckPath(string path, bool all = false)
        {
            ...
            {
                if (pth.IndexOf(FixPath(string.Concat("/Content/UserFile/", LoginUser.Id))) == 0)
                {
                    return;
                }
            }
        ...
        private ArrayList GetListFolderUser(string path, string type, bool allowUpload = false)
        {
            ...
            foreach (var dir in dirs)
            {
                ...
                try
                {
                    var guid = int.Parse(name);
                    if (LoginUser.Id == guid) continue;
